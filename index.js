class Moment {
  ...
  yearWeek() {
    ...
    let diff =
      this.time -
      new Date(`${year}-1-1`) +
      ((new Date(`${year}-1-1`).getDay() || 7) + 1) * 1000 * 60 * 60 * 24;
  ...
  startDay(type) {
    ...
    switch (type) {
      case "Y":
        ...
        break;
      case "Q":
        result = `${year}-${`0${-2 + quarter * 3}`.slice(-2)}-01`;
        break;
      case "M":
        result = `${year}-${`0${month}`.slice(-2)}-01`;
        break;
      case "w":
        result = new Moment(this.time)
          .add(-this.time.getDay() + 1, "d")
          .format();
        break;
      default:
  ...
  endDay(type) {
    ...
    switch (type) {
      case "Y":
        ...
        break;
      case "Q":
        result = `${year}-${`0${quarter * 3}`.slice(-2)}-${new Date(
          year,
          quarter * 3,
          0
        ).getDate()}`;
      ...
      case "M":
        ...
        result = `${year}-${`0${month}`.slice(-2)}-${date}`;
        break;
      case "w":
        result = new Moment(this.time)
          .add(7 - this.time.getDay(), "d")
          .format();
        break;
      default:
        console.warn(`startDay 错误`);
    ...
    for (let key of Object.keys(option)) {
      ...
      if (ret) {
        format = format.replace(
          ret[1],
          option[key].substring(option[key].length - ret[1].length)
        );
      }
    }
    return format;
